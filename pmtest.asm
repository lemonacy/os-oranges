...
LABEL_MEM_CHK_OK:
    ...
    ; 关中断
    cli     ; 在执行lidt之前用cli指令清IF位，暂时不影响可屏蔽中断
...
LABEL_REAL_ENTRY:   ; 从保护模式调回到实模式就到了这里
    ...
    mov     al,     [_SavedIMREG]
    out     021h,   al              ; 恢复中断屏蔽寄存器(IMREG)的原值
...
Init8259A:
    ...
    mov     al,     020h        ; IRQ0对应中断向量号0x20
    out     021h,   al          ; 主8259A，ICW2
    call    io_delay

    mov     al,     028h        ; IRQ8对应中断向量号0x28
